...
export default function Conference({ config, onDisconnect }: ConferenceProps) {
  ...
  // Use callback refs for more reliable attachment
  const localVideoRef = useCallback((element: HTMLVideoElement | null) => {
    if (element && localVideoTrack && !localVideoAttached) {
      console.log('[Video] Attaching local video track to element');
      try {
        localVideoTrack.attach(element);
        element.muted = true;
        setLocalVideoAttached(true);
      } catch (err) {
        console.error('[Video] Failed to attach local video:', err);
      }
    }
  }, [localVideoTrack, localVideoAttached]);

  const remoteVideoRef = useCallback((element: HTMLVideoElement | null) => {
    if (element && remoteVideoTracks.length > 0 && !remoteVideoAttached) {
      const remoteVideo = remoteVideoTracks[0];
      console.log(`[Video] Attaching remote video from ${remoteVideo.participantName}`);
      try {
        remoteVideo.track.attach(element);
        element.muted = true;
        setRemoteVideoAttached(true);
      } catch (err) {
        console.error('[Video] Failed to attach remote video:', err);
      }
    }
  }, [remoteVideoTracks, remoteVideoAttached]);
  ...
  useEffect(() => {
    return () => {
      ...
      remoteVideoTracks.forEach(v => v.track.detach());
    };
  }, []);
  ...
  return (
    <div className="fixed inset-0 bg-slate-900 flex flex-col">
      ...
      <div className="flex-1 relative">
        {/* Remote Video (Full Screen) - ALWAYS RENDER, hide with CSS */}
        <video
          ref={remoteVideoRef}
          autoPlay
          playsInline
        ...
        {!hasRemoteVideo && (
          <div className="w-full h-full bg-gradient-to-br from-slate-800 via-slate-900 to-slate-800 flex items-center justify-center">
            <div className="text-center">
              ...
              <p className="text-gray-500 text-sm mb-4">Share the room name to invite participants</p>
              <div className="inline-flex items-center gap-2 bg-slate-800 px-4 py-2 rounded-xl">
                <span className="text-purple-400 font-mono">{config.roomName}</span>
                <button 
                  onClick={() => navigator.clipboard.writeText(config.roomName)}
        ...
        {remoteParticipant && hasRemoteVideo && (
          <div className="absolute top-20 left-4 bg-black/40 backdrop-blur-sm px-3 py-1.5 rounded-lg">
            <div className="flex items-center gap-2">
              ...
              <div>
                <p className="text-white font-medium text-sm">{remoteParticipant.name}</p>
                <p className="text-gray-400 text-xs">
                  {remoteParticipant.isSpeaking ? '🎙️ Speaking' : '🔇 Silent'}
                </p>
              </div>
            </div>
          </div>
        ...
        <div className={`absolute bottom-28 right-4 w-48 aspect-video rounded-xl overflow-hidden shadow-2xl border-2 transition-all duration-300 ${
          ...
          <video
            ...
            playsInline
            muted
            className={`w-full h-full object-cover ${hasLocalVideo ? '' : 'hidden'}`}
